...
processor_chip_type  processor_chip_list[] = {
   ...
   { 0x00000001, 5, "Atheros AR5315 CPU" },                 // AR5315 chip - Not Completely Verified Yet
   { 0, 0, 0 }
   };
...
void set_instr(int instr)
{
	...
	for (i = 0; i < instruction_length; i++)
    {
		if(i == (instruction_length - 1)) {
			...
			outBuffer[buffCount++] = 0x1A;
			outBuffer[buffCount++] = 0; // Length
			outBuffer[buffCount++] = (instr >> i) & 1; // Byte1
		} else {
			outBuffer[buffCount++] = 0x1A;
			outBuffer[buffCount++] = 0; // Length
			outBuffer[buffCount++] = (instr >> i) & 1; // Byte1
...
static unsigned int InstReadWriteData(int instr, unsigned int in_data)
{
    int i, t;
	...
	for (i = 0; i < 32; i++)
    {
		if(i == 31) {
			...
			outBuffer[buffCount++] = 0; // Length
			outBuffer[buffCount++] = (in_data >> i) & 1; // Byte1
		} else {
			outBuffer[buffCount++] = 0x3B;
			outBuffer[buffCount++] = 0; // Length
			outBuffer[buffCount++] = (in_data >> i) & 1; // Byte1
...
//		usleep(1000);
	...
	if(ftStatus != FT_OK) {
		...
		for (i = 0; i < 32; i++)
		{
			out_data |= (pbyRxBuffer[i] >> 7) << i;
		}
	}
    return out_data;
}

static unsigned int InstReadWriteData2(int instr1, unsigned int in_data1,
	int instr2, unsigned int in_data2)
{
    int i, t;
    unsigned int out_data = 0;
    unsigned char out_bit;
	...
	for (i = 0; i < instruction_length; i++)
    {
		if(i == (instruction_length - 1)) {
			...
			outBuffer[buffCount++] = 0x1A;
			outBuffer[buffCount++] = 0; // Length
			outBuffer[buffCount++] = (instr1 >> i) & 1; // Byte1
		} else {
			outBuffer[buffCount++] = 0x1A;
			outBuffer[buffCount++] = 0; // Length
			outBuffer[buffCount++] = (instr1 >> i) & 1; // Byte1
	...
	for (i = 0; i < 32; i++)
    {
		if(i == 31) {
			...
			outBuffer[buffCount++] = 0x1A;
			outBuffer[buffCount++] = 0; // Length
			outBuffer[buffCount++] = (in_data1 >> i) & 1; // Byte1
		} else {
			outBuffer[buffCount++] = 0x1A;
			outBuffer[buffCount++] = 0; // Length
			outBuffer[buffCount++] = (in_data1 >> i) & 1; // Byte1
	...
	for (i = 0; i < instruction_length; i++)
    {
		if(i == (instruction_length - 1)) {
			...
			outBuffer[buffCount++] = 0x1A;
			outBuffer[buffCount++] = 0; // Length
			outBuffer[buffCount++] = (instr2 >> i) & 1; // Byte1
		} else {
			outBuffer[buffCount++] = 0x1A;
			outBuffer[buffCount++] = 0; // Length
			outBuffer[buffCount++] = (instr2 >> i) & 1; // Byte1
	...
	for (i = 0; i < 32; i++)
    {
		if(i == 31) {
			...
			outBuffer[buffCount++] = 0x3B;
			outBuffer[buffCount++] = 0; // Length
			outBuffer[buffCount++] = (in_data2 >> i) & 1; // Byte1
		} else {
			outBuffer[buffCount++] = 0x3B;
			outBuffer[buffCount++] = 0; // Length
			outBuffer[buffCount++] = (in_data2 >> i) & 1; // Byte1
...
//    WriteData(addr);
	InstReadWriteData(INSTR_ADDRESS, addr);

    // Initiate DMA Read & set DSTRT
//    set_instr(INSTR_CONTROL);
//    ctrl_reg = ReadWriteData(DMAACC | DRWN | DMA_WORD | DSTRT | PROBEN | PRACC);
	ctrl_reg = InstReadWriteData(INSTR_CONTROL, (DMAACC | DRWN | DMA_WORD | DSTRT | PROBEN | PRACC));
//	ctrl_reg = InstReadWriteData2(INSTR_ADDRESS, addr, INSTR_CONTROL, (DMAACC | DRWN | DMA_WORD | DSTRT | PROBEN | PRACC));
	usleep(1000*100);
    ...
    data = ReadData();
//	data = InstReadWriteData(INSTR_DATA, 0x00);
//	usleep(1000*100);

    // Clear DMA & Check DERR
//    set_instr(INSTR_CONTROL);
//    if (ReadWriteData(PROBEN | PRACC) & DERR)
	if (InstReadWriteData(INSTR_CONTROL, (PROBEN | PRACC)) & DERR)
    {
        if (retries--) {
			printf("DMA retry\n");
...
//    ctrl_reg = ReadWriteData(DMAACC | DRWN | DMA_HALFWORD | DSTRT | PROBEN | PRACC);
	ctrl_reg = InstReadWriteData(INSTR_CONTROL, DMAACC | DRWN | DMA_HALFWORD | DSTRT | PROBEN | PRACC);
//	ctrl_reg = InstReadWriteData2(INSTR_ADDRESS, addr, INSTR_CONTROL, DMAACC | DRWN | DMA_HALFWORD | DSTRT | PROBEN | PRACC);
